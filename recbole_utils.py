...
class RecUtils:
    ...
    def _get_recommandation_list_with_scores(self, user_id_token, topk, model_file):
        """
        获取用户的Top-K推荐物品及其分数
        
        Args:
            user_id_token: 编码后的用户ID
            topk: 推荐数量
            model_file: 模型文件路径
            
        Returns:
            tuple: (item_token_list, scores_list) 推荐物品ID列表和对应的分数列表
        """
        checkpoint = torch.load(model_file, weights_only=False, map_location=self.config["device"])
        self.model.load_state_dict(checkpoint["state_dict"])
        self.model.load_other_parameter(checkpoint.get("other_parameter"))
        ...
        topk_scores, result_tensor = torch.topk(scores, topk, dim=-1)
        return result_tensor, topk_scores

    def get_recommandation_list_with_scores(self, ori_user_id, topk, model_file):
        """
        传入原始用户id，返回推荐的原始itemid和对应的分数
        ...
        Returns:
            list: 包含(item_id, score)元组的列表
        """
        user_id_token = self.get_encode_user_token(ori_user_id)
        item_token_list, scores_list = self._get_recommandation_list_with_scores(user_id_token, topk, model_file)

        rec_result = []
        for i, item_token in enumerate(item_token_list):
            item_id = self.get_ori_item_id(item_token.item())
            score = scores_list[i].item()
            rec_result.append((item_id, score))
