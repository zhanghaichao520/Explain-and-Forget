...
class RandomExplainer:
    ...
    def explain(self, user_id_str, target_item_str, k=5):
        ...
        try:
            ...
            # 随机选择k个历史交互作为解释
            if len(user_history) <= k:
                selected_history = user_history
            else:
                # 随机选择k个
                selected_indices = np.random.choice(len(user_history), k, replace=False)
                selected_history = user_history.iloc[selected_indices]
            
            # 为每个选中的历史交互随机生成重要性得分
            explanation_results = []
            for idx, row in selected_history.iterrows():
                explanation_results.append({
                    'item_id': row['item_id'],
                    'importance': np.random.random()  # 随机生成[0,1)之间的重要性得分
                })
...
class GradOnlyExplainer:
    ...
    def compute_gradients(self, user_id_token, item_id_token, user_id_str):
        """
        计算梯度归因
        ...
        # 计算梯度：对于点积 u^T * v，梯度是 v
        # 这里我们计算每个历史物品嵌入对最终分数的贡献梯度
        gradients = []
        for i in range(len(history_embeddings)):
            # 简化的梯度计算：物品嵌入与目标物品嵌入的点积
            grad = torch.dot(history_embeddings[i], item_embedding)
            gradients.append(grad.item())
        
        return np.array(gradients), history_item_ids
    ...
    def explain(self, user_id_str, target_item_str, k=5):
        ...
        try:
            ...
            # 计算梯度
            gradients, history_item_ids = self.compute_gradients(user_id_token, target_item_token, user_id_str)
...
class PerturbOnlyExplainer:
    ...
    def optimize_perturbation(self, user_id_token, item_id_token, user_id_str, iterations=50, lr=0.1):
        ...
        for i in range(iterations):
            ...
            aggregated_neighbors_embedding = torch.mean(weighted_history_embeddings, dim=0)
...
def find_model_file(rec_utils, model_name, dataset_name):
    # 遍历文件夹中的所有文件
    for filename in os.listdir(rec_utils.config["checkpoint_dir"]):
        # 检查文件名是否包含模型名和数据集名
        if model_name in filename and dataset_name in filename:
            return os.path.join(rec_utils.config["checkpoint_dir"], filename)
...
def main():
    ...
    print(f"使用的策略: {EXPLANATION_STRATEGY}")
    print("=" * 50)
    
    try:
        # 根据策略创建相应的解释器
        if EXPLANATION_STRATEGY == "random":
            print("  使用随机解释器")
            explainer = RandomExplainer(rec_utils)
        elif EXPLANATION_STRATEGY == "grad-only":
            print("  使用梯度解释器")
            explainer = GradOnlyExplainer(rec_utils, model_file)
        elif EXPLANATION_STRATEGY == "perturb-only":
            print("  使用扰动仅解释器")
            explainer = PerturbOnlyExplainer(rec_utils, model_file)
        else:  # 默认使用CEU
            print("  使用CEU解释器")
        ...
        for user_idx, user_id_str in enumerate(user_ids):
            ...
            # if (user_idx + 1) % 100 == 0:
                # save_all_explanations_to_file(all_explanations, f"{DATASET}_{MODEL}_all_counterfactual_explanations_{EXPLANATION_STRATEGY}_partial.json")
        
        # 保存所有解释结果到文件
        save_all_explanations_to_file(all_explanations, f"{results_dir}/{DATASET}_{MODEL}_all_counterfactual_explanations_{EXPLANATION_STRATEGY}.json")
...
if __name__ == "__main__":
    # 添加命令行参数解析
    parser = argparse.ArgumentParser(description="生成反事实解释")
    parser.add_argument("--strategy", type=str, default="perturb-only", 
                        choices=["CEU", "random", "grad-only", "perturb-only"],
                        help="选择解释策略: CEU(默认), random, grad-only, perturb-only")
    parser.add_argument("--user", type=str, help="指定用户ID进行单个解释生成")
    parser.add_argument("--item", type=str, help="指定物品ID进行单个解释生成")
    args = parser.parse_args()
    
    # 设置策略
    if args.strategy:
        EXPLANATION_STRATEGY = args.strategy
    
    if args.user and args.item:
        # 为指定用户和物品生成解释
        explain_for_specific_user_item(args.user, args.item, k=5)
    else:
        # 为所有用户生成解释
        main()
