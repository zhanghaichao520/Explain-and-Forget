...
def main():
    # 指定包含unlearned推荐结果文件的文件夹
    results_dir = f"{DATASET}_{MODEL}_alpha_experiment_results"
    
    # 获取文件夹下所有JSON文件
    unlearned_rec_files = glob.glob(os.path.join(results_dir, "*.json"))
    unlearned_rec_files.sort()  # 按文件名排序
    
    # 如果没有找到文件，给出提示
    if not unlearned_rec_files:
        print(f"在 {results_dir} 文件夹中未找到任何JSON文件")
        return
    
    print(f"找到 {len(unlearned_rec_files)} 个unlearned推荐结果文件")
    ...
    # 为每个unlearned文件计算MIA结果
    mia_results = {}
    
    for unlearned_rec_file in unlearned_rec_files:
        print(f"\n处理文件: {unlearned_rec_file}")
        
        # 提取alpha值用于标识
        filename = os.path.basename(unlearned_rec_file)
        alpha_value = filename.split('_alpha')[-1].replace('.json', '')
        
        # 加载unlearned推荐结果
        unlearned_recommendations = load_recommendations(unlearned_rec_file)
        print(f"  加载推荐结果用户数: {len(unlearned_recommendations)}")
        
        # 转换为编码后的形式
        print("  正在转换推荐结果编码...")
        encoded_unlearned_recommendations = convert_recommendations_to_encoded(unlearned_recommendations)
        print(f"  编码后推荐用户数: {len(encoded_unlearned_recommendations)}")
        
        # 评估遗忘效果（仅计算M_unlearned的结果）
        forget_results = evaluate_unlearned_effectiveness(
            attack_model, scaler, forget_interactions,
            encoded_unlearned_recommendations,
            encoded_forget_interactions, encoded_test_interactions, encoded_retain_interactions, k=50
        )
        
        # 存储结果
        mia_results[alpha_value] = forget_results
        print(f"  Alpha={alpha_value} 的MIA结果: ACC={forget_results['ACC']:.4f}, AUC={forget_results['AUC']:.4f}")
    
    # 输出所有结果
    print("\n所有Alpha值的MIA评估结果:")
    print("=" * 50)
    for alpha_value, metrics in mia_results.items():
...
def evaluate_unlearned_effectiveness(clf, scaler, forget_interactions, 
                                   recommendations_munlearned,
                                   encoded_forget_interactions, encoded_test_interactions, encoded_retain_interactions, k=50):
    """
    仅评估遗忘后模型(M_unlearned)的效果
    :param clf: 训练好的攻击模型
    :param scaler: 特征标准化器
    :param forget_interactions: 遗忘集用户交互（原始ID）
    :param recommendations_munlearned: 遗忘后模型推荐结果（编码后ID）
    ...
    :param encoded_retain_interactions: 编码后的保留集用户交互
    :param k: Top-K值
    :return: M_unlearned模型在遗忘集上的ACC和AUC
    """
    encoded_forget_users = list(encoded_forget_interactions.keys())
    encoded_test_users = list(encoded_test_interactions.keys())
    encoded_retain_users = list(encoded_retain_interactions.keys())
    ...
    if X_munlearned_acc_positive and X_munlearned_acc_negative:
        ...
        return {'ACC': acc_munlearned, 'AUC': auc_munlearned, 'pred_mean': np.mean(y_pred_proba_munlearned_acc)}
    
    # 如果没有数据，返回默认值
    return {'ACC': 0.5, 'AUC': 0.5, 'pred_mean': 0.5}
