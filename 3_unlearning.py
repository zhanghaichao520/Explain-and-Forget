...
def split_trainset_for_unlearning(trainset, forget_ratio=0.01, random_state=42):
    ...
    Args:
        ...
        forget_ratio: 需要遗忘的数据比例，默认为0.01(1%)
...
# 保存更新后的推荐结果到新文件
updated_output_file = f"{DATASET}_{MODEL}_recommendations_top{TOPK}_updated.json"
...
# 评估遗忘学习效果
evaluation_results = evaluate_unlearning_effect(recommendations, adjusted_results, topk=TOPK)


# 显示评估摘要
print(f"\n=== 遗忘学习效果评估摘要 ===")
print(f"处理用户数: {evaluation_results['total_users']}")
print(f"产生变化的用户数: {evaluation_results['users_with_changes']}")
print(f"总推荐项数: {evaluation_results['total_items']}")
print(f"发生变化的推荐项数: {evaluation_results['changed_items']}")
print(f"平均变化率: {evaluation_results['average_change_rate']:.4f}")

# 显示一些示例结果
if adjusted_results:
    example_user = list(adjusted_results.keys())[0]
    example_result = adjusted_results[example_user]
    print(f"\n示例用户 {example_user} 的调整后推荐列表:")
    print(f"遗忘物品数量: {example_result['forget_items_count']}")
    print(f"遗忘物品列表: {example_result['forget_items']}")
    
    # 处理不同格式的推荐结果
    original_user_recs = recommendations[str(example_user)]
    if isinstance(original_user_recs, dict):
        # 新格式：{item_id: score, ...}，按分数排序提取前5个物品ID
        sorted_items = sorted(original_user_recs.items(), key=lambda x: x[1], reverse=True)
        original_rec_sample = [item_id for item_id, score in sorted_items][:5]
    else:
        # 旧格式：[item_id, ...]
        original_rec_sample = original_user_recs[:5]
        
    print(f"原始推荐列表: {original_rec_sample}...")
    print(f"调整后推荐列表: {example_result['adjusted_recommendations'][:5]}...")